...
CLAUDE_MODEL = "vertex/claude-4.5-opus"
GEMINI_MODEL = "google/gemini-3-pro"
GEMINI_FLASH_MODEL = "google/gemini-3-flash"
...
def spawn_cli_agent(config: AgentConfig, prompt: str) -> RunningAgent:
    ...
    if stdin_payload is not None and process.stdin:
        ...
        process.stdin.close()
    return RunningAgent(
        config=config, prompt=prompt, start_time=time.time(), process=process
    )
...
def _ui_upsert_planner(
    ...
    def mutator(state: Dict[str, Any]) -> None:
        ...
        if not isinstance(planners, list):
            planners = []
        index = next(
            (i for i, item in enumerate(planners) if item.get("id") == planner_id), None
        )
        if index is None:
...
def _ui_update_judge(
    ...
    ui_state.mutate(mutator)
    _ui_emit(
        ui_instance, "judge_update", {"judge": judge_entry, "timestamp": timestamp}
    )
...
def _ui_set_final_plan(
    ...
    ui_state.mutate(mutator)
    _ui_emit(
        ui_instance, "final_plan", {"final_plan": final_plan, "timestamp": timestamp}
    )
...
def _build_refine_prompt(
    plan_template: str, task_brief: str, final_plan: str, context: str
) -> str:
...
def _rebuild_ui_state_from_run(run_dir: Path) -> Dict[str, Any]:
    ...
    for plan_path in sorted(run_dir.glob("plan-*.md")):
        ...
        if (
            name.endswith("-attempt1")
            or name.endswith("-attempt2")
            or name.endswith("-attempt3")
        ):
            continue
        planners.append(
            {
        ...
        try:
            ...
            if path == "/api/refine":
                ...
                prompt = _build_refine_prompt(
                    plan_template, task_brief, final_plan, context
                )
                ...
                try:
                    ...
                    _ui_action_result(
                        ui_instance, "refine", "failed", str(exc), None, _ui_timestamp()
                    )
                    continue
            ...
            if path == "/api/keepalive":
                ...
                deadline = (
                    ""
                    if keep_open
                    else _ui_deadline_from_now(DEFAULT_UI_SESSION_TTL_SEC)
                )
                _ui_set_session_state(
                    ui_state, ui_instance, keep_open, deadline, _ui_timestamp()
                )
            ...
            _ui_action_result(
                ...
                "unknown",
                "ignored",
                f"unhandled action: {path}",
                None,
                _ui_timestamp(),
            )
        except Exception as exc:
            _ui_action_result(
                ui_instance, "error", "failed", str(exc), None, _ui_timestamp()
            )


def render_planner_prompt(
    task_spec: Dict[str, Any], plan_template: str, prompt_template: str
) -> str:
...
def configure_agents(config_path: Path) -> None:
    ...
    print("Council setup")
    if prompt_yes_no(
        "Use default council (Gemini 3 Pro + Gemini 3 Flash + Claude 4.5 Opus)?",
        default_yes=True,
    ):
        planners = [
            {"name": "gemini-pro", "kind": "opencode", "model": GEMINI_MODEL},
            {"name": "gemini-flash", "kind": "opencode", "model": GEMINI_FLASH_MODEL},
            {"name": "claude-opus", "kind": "opencode", "model": CLAUDE_MODEL},
        ]
        judge = planners[2]
    else:
        ...
        for idx in range(1, planner_count + 1):
            ...
            if kind == "codex":
                ...
                planner["reasoning_effort"] = prompt_text(
...
def _normalize_agent_spec(spec: Any, fallback_name: str) -> AgentConfig:
    ...
    kind = (
        str(data.get("kind") or data.get("cli") or data.get("type") or name)
        .strip()
        .lower()
    )
...
def load_agent_configs(
    task_spec: Dict[str, Any], config_path: Optional[Path] = None
) -> Tuple[List[AgentConfig], AgentConfig]:
    ...
    while remaining and attempt <= RETRY_LIMIT:
        ...
        for planner in remaining:
            prompt = render_planner_prompt(
                task_spec, plan_template, planner_prompt_template
            )
            timestamp = _ui_timestamp()
            _ui_upsert_planner(
                ui_state,
        ...
        for entry in running:
            ...
            plan_path = (
                Path(run_dir) / f"plan-{entry.config.name}-attempt{attempt + 1}.md"
            )
            ...
            timestamp = _ui_timestamp()
            status = (
                "complete" if valid else ("failed" if timeout_error else "needs-fix")
            )
            errors = [err] if err else []
            summary = plan_text
            _ui_upsert_planner(
            ...
            # Use final_path if valid, otherwise plan_path (which is always set if write_attempt is True)
            # If write_attempt is False (valid=True, attempt=0), final_path is set.
            # If write_attempt is True, plan_path is set.
            saved_path = str(plan_path if write_attempt else final_path)
            result = AgentResult(
            ...
            result = AgentResult(
                ...
                data={
                    "path": str(
                        plan_path if write_attempt else (final_path or plan_path)
                    ),
                    "text": plan_text,
                },
                valid=valid,
...
def run_judge(
    ...
    prompt = render_judge_prompt(
        task_spec, plans, judge_template, judge_prompt_template
    )
...
def main() -> int:
    ...
    ui.add_argument(
        "--run-dir", required=True, help="Path to a run directory to resume"
    )
    ui.add_argument(
    ...
    configure.add_argument(
        "--config", required=False, help="Path to write agents config JSON"
    )
    ...
    if args.cmd == "ui":
        ...
        if not args.no_open and ui_url:
    ...
    planner_prompt = (
        prompt_text.split("## Judge Prompt")[0]
        .split("```text", 1)[1]
        .rsplit("```", 1)[0]
    )
    judge_prompt = (
        prompt_text.split("## Judge Prompt", 1)[1]
        .split("```text", 1)[1]
        .rsplit("```", 1)[0]
    )
    ...
    if ui_state:
        timestamp = _ui_timestamp()
        initial_planners = [
            {"id": planner.name, "status": "pending", "summary": "", "errors": []}
        ...
        _ui_emit(
            ui_instance, "phase_change", {"phase": "starting", "timestamp": timestamp}
        )
    ...
    final_text = extract_final_plan(
        judge_result.data.get("text", "") if judge_result.data else ""
    )
    final_path = run_dir / "final-plan.md"
    ...
    if ui_instance and args.ui_keepalive_seconds > 0:
        print(
            f"Keeping UI server alive for {args.ui_keepalive_seconds}s unless kept open..."
        )
        start = time.time()
        while True:
            if keepalive and keepalive.should_keep_open():
