...
class _UIRequestHandler(SimpleHTTPRequestHandler):
    ...
    def list_directory(self, path: str) -> Optional[Any]:  # type: ignore
...
def start_server(
    ui_dir: Optional[Path] = None, state: Optional[UIState] = None
) -> UIServer:
    ui_path = ui_dir or (Path(__file__).resolve().parent / "ui")
    server = UIServer(ui_path, state=state)
    thread = threading.Thread(
        target=server.serve_forever, name="ui-server", daemon=True
    )
    thread.start()
    return server
